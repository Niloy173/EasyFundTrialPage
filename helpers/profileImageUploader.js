...
const UPLAOD_FOLDER = path.join(__dirname + "/../" + "/public/profilePicture/");

function check() {
  fs.readdir(UPLAOD_FOLDER, (err, files) => {
    for (const file of files) {
      fs.unlink(path.join(UPLAOD_FOLDER, file), (err) => {
        if (err) new Error("File Error"); // Not efficient
      });
    }
  });
}
...
var upload = multer({
  storage: storage,
  limits: {
    fileSize: 50000000, // size may vary
  },
